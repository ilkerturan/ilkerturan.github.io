...
### Modern SQL Standardı (FETCH FIRST / OFFSET)

SQL Server (2012+) ve Oracle (12c+) dâhil birçok modern veritabanı artık ANSI SQL standardı olan `OFFSET` ve `FETCH NEXT` (veya `FETCH FIRST`) komutlarını desteklemektedir:
...
### SQL Server'da TOP (Eski/Alternatif Kullanım)
...
### Oracle'da ROWNUM (Eski Kullanım)

12c öncesi Oracle sürümlerinde LIMIT yerine `ROWNUM` kullanılırdı:
