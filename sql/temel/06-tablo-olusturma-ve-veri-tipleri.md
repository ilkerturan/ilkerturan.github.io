...
### JSON Veri Tipi (Modern Veritabanları)

MySQL (5.7+), PostgreSQL ve SQL Server (2016+) gibi modern veritabanları artık `JSON` formatını yerel olarak desteklemektedir. NoSQL esnekliğini ilişkisel veritabanlarına taşır.
...
-- SELECT config->>'tema' FROM ayarlar;
```
...
1. **WHERE sorgularında sık kullanılan sütunlara indeks ekleyin**2.**JOIN işlemlerinde kullanılan sütunlara indeks ekleyin**3.**Küçük tablolara indeks eklemeyin**4.**Çok fazla indeks eklemeyin** (ekleme/güncelleme yavaşlar)
