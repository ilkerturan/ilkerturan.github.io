...
- **Encapsulation (Kapsülleme)**-**Inheritance (Kalıtım)**-**Polymorphism (Çok Biçimlilik)**-**Abstraction (Soyutlama)**
...
Interface, sınıfların uygulaması gereken bir sözleşme tanımlar. Sınıflar birden fazla interface implement edebilir. 

**Not (C# 8.0+):** C# 8.0 ile birlikte "Default Interface Methods" özelliği gelmiştir. Artık interface içinde gövdesi olan varsayılan metotlar tanımlanabilmektedir.
...
public interface IUcabilir
{
    ...
    void BakimYap()
    {
        Console.WriteLine("Uçan aracın bakımı yapılıyor (Varsayılan davranış).");
    }
}
